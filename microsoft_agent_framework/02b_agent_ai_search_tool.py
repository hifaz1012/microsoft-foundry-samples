...
to search through indexed blackrock fund asset data and answer user questions about blackrock fund assets.

Prerequisites:
1. Set AZURE_AI_PROJECT_ENDPOINT and AZURE_AI_MODEL_DEPLOYMENT_NAME environment variables
2. Ensure you have an Azure AI Search connection configured in your Azure AI project
3. The search index "ai-search-aas-test-1" should exist in your Azure AI Search service
   (you can create this using the Azure portal with sample fund asset data)
...
async def main() -> None:
    ...
    async with (
        ...
        azure_ai_agent = await agents_client.as_agent(
            ...
            instructions=(
                "You are a helpful agent that searches fund asset information using Azure AI Search. "
                "Always use the search tool and index to find blackrock product data and provide accurate information."
            ),
                    ...
                    "indexes": [
                        {
                            ...
                            "index_name": "ai-search-aas-test-1",
                            "query_type": "vector",
                        }
                    ]
                }
        ...
        try:
            async with ChatAgent(
                ...
                instructions=("You are a helpful agent that uses the search tool and index to find fund asset information."),
            ) as agent:
                print("This agent uses raw Azure AI Search tool to search fund asset data.\n")

                # 3. Simulate conversation with the agent
                user_input = (
                    "Summarize key information about Blackrock product asset"
                ...
                print("\n" + "=" * 50 + "\n")
                print("Fund asset search conversation completed!")
